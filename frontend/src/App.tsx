...
function App() {
  ...
  const [selectedBoardId, setSelectedBoardId] = useState<number>(localStorage.getItem(BOARD_ID_KEY) ? Number(localStorage.getItem(BOARD_ID_KEY)) : 0);
  ...
  const setCurrentBoard = (boardId: number) => {
    ...
    boardId ? localStorage.setItem(BOARD_ID_KEY, boardId.toString()) : localStorage.removeItem(BOARD_ID_KEY);
  }
